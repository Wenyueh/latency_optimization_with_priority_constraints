...
async def GPU_execute(arrival_time_id, arrival_intervals, full_queue, ongoing_request, record_requests):
    ...
    if arrival_time_id < len(arrival_intervals) - 1:
        ...
        if running_time_for_this_interval > full_time_required:
            while running_time_for_this_interval > full_time_required and len(full_queue.nodes) !=0:
...
async def simulator(args, user_requests):
    ...
    for arrival_time_id, arrived_user_request_ids in enumerate(list(time_arrived_requests.values())):
        ...
        if ongoing_request is None:
            ...
            # get the next node to process --> max operation
        ...
        # try:
        #     await asyncio.gather(GPU_execute(arrival_time_id, arrival_intervals, full_queue, ongoing_request, record_requests), incremental_update())
        # except asyncio.CancelledError:
        #     incremental_update_task = [t for t in asyncio.all_tasks() if not t.done() and t.get_name().startswith('incremental_update') and not t.cancelled()]
        #     if incremental_update_task:
        #         await cancel(incremental_update_task)
