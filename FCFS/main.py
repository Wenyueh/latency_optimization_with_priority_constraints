...
async def simulator(args, user_requests):
    ...
    simulated_predicted_priority = priority_by_arrival(args, user_requests) #simulated_priority_predictor(args, oracle_predicted_priority)
...
async def GPU_execute(MaxHeap_Memory, full_queue):
    def one_execute_iteration(ongoing_requests):
        ...
        if prefilling_requests:
            ...
            if not is_sorted(prefilling_requests):
                ...
                prefilling_requests.sort(key=lambda x: x.predicted_priority)
        else:
            is_prefilling = False
    ...
    def call_scheduler(args, full_queue):
        ...
        if next_node.predicted_priority <= config.ongoing_requests[0].predicted_priority:
        ...
        def merge_with_type(ongoing_requests, next_batch, require_decode=require_decode, max_prefilling_time=max_prefilling_time):
            def merge(list1, list2):
                ...
                while i < len(list1) and j < len(list2):
                    if list1[i].predicted_priority <= list2[j].predicted_priority:
                        pq.append(list1[i])
                        i += 1
                    else:
                        pq.append(list2[j])
