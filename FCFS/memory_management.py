        ...
        if self.storage_left() >= requested_blocks:
            ...
            while requested_blocks_remaining > 0:
                ...
                if lowest_priority_request_id == request_id:
                    ...
                    # if there's only one request being processed and it is the same as the preempted request
                    # then there won't be any update on time, thus we need to slightly modify 
                    modification = False
                    if lowest_priority_request.predicted_remaining_computation_time[0] > 0:
                        if len([r for r in config.ongoing_requests if r.predicted_remaining_computation_time[0] > 0]) == 1:
                            modification = True
                    if lowest_priority_request.predicted_remaining_computation_time[0] <= 0:
                        if len([r for r in config.ongoing_requests if r.predicted_remaining_computation_time[0] <= 0]) == 1:
                            modification = True
