...
async def GPU_execute(MaxHeap_Memory, full_queue):
    def one_execute_iteration(ongoing_requests):
        ...
        if prefilling_requests:
            ...
            if not is_sorted(prefilling_requests):
                ...
                prefilling_requests.sort(key=lambda x: x.predicted_remaining_computation_time[0] + x.prefill_cache_loading_time + x.decoding_cache_loading_time, reverse=True)
        else:
            is_prefilling = False
    ...
    def call_scheduler(args, full_queue):
        ...
        if (next_node.predicted_remaining_computation_time[-1] + next_node.prefill_cache_loading_time + next_node.decoding_cache_loading_time) <= (config.ongoing_requests[0].predicted_remaining_computation_time[-1]  + config.ongoing_requests[0].prefill_cache_loading_time + config.ongoing_requests[0].decoding_cache_loading_time):
        ...
        def merge_with_type(ongoing_requests, next_batch, require_decode=require_decode, max_prefilling_time=max_prefilling_time):
            def merge(list1, list2):
                ...
                while i < len(list1) and j < len(list2):
                    if (list1[i].predicted_remaining_computation_time[-1] + list1[i].prefill_cache_loading_time + list1[i].decoding_cache_loading_time) <= (list2[j].predicted_remaining_computation_time[-1] + list2[j].prefill_cache_loading_time + list2[j].decoding_cache_loading_time):
                        pq.append(list1[i])
                        i += 1
                    else:
                        pq.append(list2[j])
    ...
    while has_requests:
        ...
        try:
            ...
            # for r in config.ongoing_requests:
            #     print(r.user_request_id, r.predicted_priority, r.predicted_remaining_computation_time)
            #     print(r.total_time, r.waiting_time)
            #     print('----')
            # full_queue.visualize()
            # MaxHeap_Memory.visualize()
            # print('=' * 50)
