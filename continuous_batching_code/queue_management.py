...
class TwoDimensionalPriorityQueue:
    ...
    def delete(self, item):
        ...
        item = (item.predicted_priority, item.predicted_remaining_computation_time[-1] + item.prefill_cache_loading_time + item.decoding_cache_loading_time, item)
...
class PriorityQueue:
    ...
    def fetch_next_k_nodes(self, k, require_decode=False, max_prefilling_time=float('inf')):
        ...
        # sort the nodes in a sorted array first by priority then by predicted remaining computation time from small to large
        all_nodes.sort(key=operator.attrgetter('predicted_priority', 'predicted_remaining_computation_time'))
